...
import com.navercorp.pinpoint.api.bo.SpanDecoderV0api;
...
public class HbaseTraceDaoV2 implements TraceDao {
    ...
    @PostConstruct
    private void setup() {
        SpanMapperV2 spanMapperV2 = new SpanMapperV2(rowKeyDecoder, new SpanDecoderV0api(), stringCacheSize);
    ...
    List<List<SpanBo>> selectSpans(List<GetTraceInfo> getTraceInfoList, int eachPartitionSize) {
        ...
        logger.info("partition count = " + partitionGetTraceInfoList.size());
...
//            logger.info("checking fin");
    ...
    private RowMapper<List<SpanBo>> getSpanMapper(GetTraceInfo getTraceInfo) {
        ...
        if (hint.isSet()) {
            final SpanDecoder targetSpanDecoder = new TargetSpanDecoder(new SpanDecoderV0api(), getTraceInfo);
